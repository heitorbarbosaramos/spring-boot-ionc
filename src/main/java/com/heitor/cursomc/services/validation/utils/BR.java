package com.heitor.cursomc.services.validation.utils;
...
public class BR {
	...
	public static boolean validaRG(String numDoc) {
		...
		if (numDoc.matches("\\d{2}\\.\\d{3}\\.\\d{3}-\\w{1}") || numDoc.matches("\\d{2}.?\\d{3}.?\\d{3}-\\w{1}") || numDoc.matches("\\d{9}") || numDoc.matches("\\d{8}\\w{1}")) {
